...
namespace librealsense
{
    ...
    void tm2_sensor::receive_pose_message(const interrupt_message_get_pose & pose_message)
    {
        ...
        frame_additional_data additional_data(ts.device_ts.count(), frame_num++, ts.arrival_ts.count(), depth_units, sizeof(frame_md), (uint8_t*)&frame_md, ts.global_ts.count(), 0, 0, false);
    ...
    void tm2_sensor::receive_video_message(const bulk_message_video_stream * message)
    {
        ...
        float depth_units = 0.0f;
        frame_additional_data additional_data(ts.device_ts.count(), message->rawStreamHeader.dwFrameId, ts.arrival_ts.count(), depth_units, sizeof(video_md), (uint8_t*)&video_md, ts.global_ts.count(), 0, 0, false);
    ...
    void tm2_sensor::handle_imu_frame(unsigned long long tm_frame_ts, unsigned long long frame_number, rs2_stream stream_type, int index, float3 imu_data, float temperature)
    {
        ...
        float depth_units = 0.0f;
        frame_additional_data additional_data(ts.device_ts.count(), frame_number, ts.arrival_ts.count(), depth_units, sizeof(motion_md), (uint8_t*)&motion_md, ts.global_ts.count(), 0, 0, false);
