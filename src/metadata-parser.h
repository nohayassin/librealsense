...
/** \brief Metadata fields that are utilized internally by librealsense
    ...
    *  Return it as a pointer to a base-class*/
    ...
    {
    public:
        md_depth_units_parser(Attribute S::* attribute_name, unsigned long long offset, attrib_modifyer mod) :
            _md_attribute(attribute_name), _offset(offset), _modifyer(mod) {};
        ...
        bool supports(const frame& frm) const override
        {
            return true; // TODO : check if expression is needed
        }

    private:
        md_depth_units_parser() = delete;
        md_depth_units_parser(const md_depth_units_parser&) = delete;
